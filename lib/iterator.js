...
var debug = require('debug')('loopback:mixin:iterator');
...
module.exports = function(Model, options) {
  ...
  Model.iterate = function(query, options) {
    return new IteratorCls(Model, query, _.defaults(options, options));
  };
...
IteratorCls.prototype.forEachAsync = function(fn, cb) {
  ...
  iterators.forEachAsync(self, function(err, item, next) {
    ...
    if (!self.err) {
      ...
      self.queue.push({
        item: item,
        status: {
          itemsFrom: self.itemsFrom,
          itemsTo: self.itemsTo,
    ...
    async.whilst(
      function() {
        return !self.queue.idle() && self.queue.length() > self.maxQueueLength;
      },
      function(callback) {
